...
public class ChatController {
    ...
    /** 모델을 부르지 않는다. 뼈대가 떴는지와 내 남은 사용량만 본다. */
    @GetMapping("/ping")
    public String ping(@AuthenticationPrincipal UserDetails user) {
        return "chatbot 준비됨 — " + user.getUsername()
                + " 님의 남은 사용량 " + quotaService.remaining(user.getUsername());
    }
    ...
    public Flux<ServerSentEvent<String>> streamChat(
        ...
        return chatService
                .streamChat(user.getUsername(), request.sessionIdOrDefault(), request.message())
